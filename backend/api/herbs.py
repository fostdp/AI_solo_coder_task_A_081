...
def list_herbs(
    ...
    result = [herb_to_response(h) for h in cursor]
    return result
...
def count_herbs(
    ...
    count = herbs_col.count_documents(query)
    return {"count": count}


@router.get("/{herb_id}")
def get_herb(herb_id: str):
    try:
...
def get_herb_categories():
    ...
    result = list(herbs_col.aggregate(pipeline))
...
def get_herb_natures():
    ...
    result = list(herbs_col.aggregate(pipeline))
    return [{"nature": r["_id"], "count": r["count"]} for r in result]
...
def get_herb_meridians():
    ...
    pipeline = [
        {"$unwind": "$meridians"},
        {"$group": {"_id": "$meridians", "count": {"$sum": 1}}},
        {"$sort": {"count": -1}}
    ]
    result = list(herbs_col.aggregate(pipeline))
    return [{"meridian": r["_id"], "count": r["count"]} for r in result]
